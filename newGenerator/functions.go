...
const (
	...
	insertTrackQuery        = "insert into track (title, lyrics_author, track_id, lyrics, explicit, file_address) values ($1, $2, $3, $4, $5, $6)"
	insertMusicianQuery     = "insert into musician (nickname, email, password, musician_id) values ($1, $2, $3, $4)"
	existsMusicianQuery     = "SELECT EXISTS(SELECT musician_id FROM musician WHERE nickname = $1)"
	existsTrackQuery        = "SELECT EXISTS(SELECT track_id FROM track WHERE title = $1)"
)

func GenerateMusician(db *sqlx.DB, musicians []structs.RandomMusician) {
	amount := structs.RandomAmount()

	for i := 0; i < amount; i++ {
		musician := randMusician(db, musicians)

		row, err := db.Queryx(insertMusicianQuery, musician.Nickname, musician.Email,
			musician.Password, musician.MusicianId)
...
func GenerateTrack(db *sqlx.DB, tracks []structs.RandomTrack) {
	amount := structs.RandomAmount()

	for i := 0; i < amount; i++ {
		track := randTrack(db, tracks)

		row, err := db.Queryx(insertTrackQuery, track.Title, track.LyricsAuthor,
			track.TrackId, track.Lyrics, track.Explicit, track.FileAddress)
